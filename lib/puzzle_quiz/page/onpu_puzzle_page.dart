...
class OnpuPuzzlePage extends StatelessWidget {
  final bool type;
  const OnpuPuzzlePage({super.key, required this.type});
  @override
...
class _MainParts extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    bool isAllCompleted = ref.watch(
        onpuPuzzlePageNotifierProvider.select((state) => state.isAllCompleted));
    Size screenSize = getScreenSize(context);
    // final String folder = getSizeFolderName(context);
    final String folder = getSizeFolderNameFromSize(screenSize);
...
class _GameParts extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    bool preStarted = ref.watch(
        onpuPuzzlePageNotifierProvider.select((state) => state.isPreStarted));
    return !preStarted
        ? _TitleParts(type)
        : Stack(
            children: [
    ...
    return Stack(
      children: [
        ...
        Positioned(
          ...
          child: PuzzleButton(
            onTap: () {
              ref.read(onpuPuzzlePageNotifierProvider.notifier).titleStart();
            },
...
class _BackGround extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    int level = ref
        .watch(onpuPuzzlePageNotifierProvider.select((state) => state.level));
    Size size = getScreenSize(context);
    String sizeFolder = getSizeFolderName(context);
...
class _ButtonParts extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final isStarted = ref.watch(
        onpuPuzzlePageNotifierProvider.select((state) => state.isStarted));
        ...
        Positioned(
          ...
          child: isStarted
              ...
              : PuzzleButton(
                  onTap: () {
                    ref
                        .read(onpuPuzzlePageNotifierProvider.notifier)
                        .start(context, type);
                  },
...
class _PieceParts extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final isCompleted = ref.watch(
        onpuPuzzlePageNotifierProvider.select((state) => state.isCompleted));

    List<OnpuPieceState> states = isStarted
        ? ref
            .read(onpuPuzzlePageNotifierProvider.notifier)
    ...
    final level =
        ref.read(onpuPuzzlePageNotifierProvider.select((state) => state.level));
...
class _TitlePieceParts extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final List<OnpuPieceState> states = ref
        .read(onpuPuzzlePageNotifierProvider.notifier)
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return isCompleted
        ? Positioned(
            ...
            child: PuzzleButton(
              onTap: () {
                ref.read(onpuPuzzlePageNotifierProvider.notifier).next(type);
              },
              imageName: 'assets/images/$folder/tugihe.png',
