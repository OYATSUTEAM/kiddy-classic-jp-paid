...
class OnpuPuzzleHePageStateNotifier
    extends StateNotifier<OnpuPuzzleHePageHeState> {
  int _completedCount = 0;
  int _targetCount = 0;
  List<OnpuCompleteState> _onpuStates = List.empty(growable: true);
  List<bool> _isPieceCorrectArea = List.empty(growable: true);
  List<OnpuPieceHeState> piecesInit = List.empty(growable: true);
  List<OnpuPieceHeState> pieceStates = List.empty(growable: true);
  List<OnpuPieceHeState> piecePreStartStates = List.empty(growable: true);
  ...
  void init(BuildContext context, bool type) {
    ...
    for (int i = 0; i < _targetCount; i++) {
      ...
      piecesInit.add(OnpuPieceHeState(index: indexes[i], position: piecePos));
      pieceStates.add(OnpuPieceHeState(index: indexes[i], position: piecePos));
    }
  }
  ...
  List<OnpuPieceHeState> getPieceStates(BuildContext context, bool type) {
    if (_targetCount <= 0) {
      init(context, type);
    }
  ...
  List<OnpuPieceHeState> getPieceStartStates(BuildContext context, bool type) {
    List<OnpuPieceHeState> pieceStartStates = List.empty(growable: true);
    ...
    for (int i = 0; i < num; i++) {
      if (isCorrectPiece(puzzleSettings, i)) {
        ...
        pieceStartStates
            .add(OnpuPieceHeState(index: i, position: positions[cnt++] + ofs));
      }
    }
    return pieceStartStates;
  }

  List<OnpuPieceHeState> getPiecePreStartStates(
    ...
    for (int i = 0; i < num; i++) {
      ...
      piecePreStartStates
          .add(OnpuPieceHeState(index: indexes[i], position: piecePos));
    }
    _isPreStart = true;
  ...
  Offset getCorrectPosition(
    ...
    if (!isStart) {
      ...
      List<OnpuPieceHeState> states = getPieceStartStates(context, type);
...
final onpuPuzzleHePageNotifierProvider = StateNotifierProvider<
    OnpuPuzzleHePageStateNotifier, OnpuPuzzleHePageHeState>(
  (ref) => OnpuPuzzleHePageStateNotifier(),
);
